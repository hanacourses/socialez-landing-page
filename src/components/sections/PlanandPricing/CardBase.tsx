...
const CardBase = (props: CardBaseProps) => {
    ...
    const SubFeatureIcons = () => {
        return (
            <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M8.425 8.43945L10 5.00195L11.5625 8.43945L15 10.002L11.5625 11.577L10 15.002L8.425 11.577L5 10.002L8.425 8.43945Z" fill="#0BA5EC" />
            </svg>
        )
    }
    return (
        <div className={`${className} w-full h-full min-h-[650px] bg-white p-8 rounded-2xl relative overflow-hidden ${isMostPopular ? "border-2 border-[#0172F4]" : ""}`}>
            {isMostPopular && <span className="text-xs font-medium absolute top-0 right-0 bg-[#0172F4] text-white px-4 py-2 rounded-bl-full">MOST POPULAR</span>}
